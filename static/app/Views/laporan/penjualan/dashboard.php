...
.dt-body-right {
  text-align: right;
  padding-right: 12px;
  font-variant-numeric: tabular-nums; /* biar angka sejajar rapi */
}
</style>
<div class="content">
                ...
                <div class="portlet portlet-collapsed">
                    ...
                    <div class="portlet-body">
                        <!-- BEGIN Form -->
                            ...
                            <div class="form-group row mb-3">
                                ...
                                <div class="col-sm-10">
                                    ...
                                    <select id="laporan_penjualan_carabayar" class="selectpicker" data-live-search="true">
                                        ...
                                        <option value="KARTUDEBIT">Kartu Debit</option>
                            ...
                            <div class="form-group row mb-3">
                                <label for="laporan_penjualan_sub_perusahaan" class="col-sm-2 col-form-label">Per Sub Perusahaan</label>
                                <div class="col-sm-10">
                                    <!-- BEGIN Input Group -->
                                    <select id="laporan_penjualan_sub_perusahaan" class="form-control" data-live-search="true"></select>
                                    <!-- END Input Group -->
                                </div>
                            </div>
                        <!-- END Form -->
...
$(document).ready(function () {
    ...
    $('#laporan_penjualan_sub_perusahaan').select2({
        allowClear: true,
        placeholder: 'Filter Berdasarkan Sub Perusahaan',
        ajax: {
            url: baseurljavascript + 'masterdata/selectvaluereport',
            method: 'POST',
            dataType: 'json',
...
function panggilreportpenjualanformat(controllerReport,jenisformatjikasama){
    ...
    swal.fire({
        ...
        if(result.isConfirmed){
            $(".tanggal_perfaktur, .carabayar_perfaktur, .pelanggan_perfaktur, .outletterpilih_perfaktur, .barang_perfaktur, .suplier_perfaktur, .group_perfaktur, .kategori_perfaktur, .subperusahaan_perfaktur").html("");
            ...
            $(".subperusahaan_perfaktur").html(($("#laporan_penjualan_sub_perusahaan").val() == null ? "SEMUA SUB PERUSAHAAN TERDAFTAR" : $("#laporan_penjualan_sub_perusahaan option:selected").text()));
            setTimeout(function() {
                if (controllerReport == "penjualandetailnota" || controllerReport == "penjualandetailtanggal"){ 
                    ...
                    let table = $("#tabel_laporan_"+controllerReport).DataTable({
                        ...
                        ajax: {
                            ...
                            "data": function (d) {
                                ...
                                d.DARISUBPERUSAHAAN = $("#laporan_penjualan_sub_perusahaan").val();
                            }
                        },
                        columns: [
                    ...
                    if (controllerReport == "penjualanperfaktur") {
                        ...
                        columnDefs.push({
                            ...
                            render: function (data, type, row) {
                                if (type === 'display' || type === 'filter') {
                                    return formatuang(data, 'id-ID', 'IDR');
                                }
                                return data;
                            }
                        });
                    ...
                    if (controllerReport == "penjualanperbarang") {
                        columnDefs.push({
                            targets: [4, 5, 6, 7],
                            className: "dt-body-right",
                            render: function (data, type, row) {
                    ...
                    if (controllerReport == "penjualanperpelanggan") {
                        columnDefs.push({
                            targets: [3, 4, 5, 6, 7, 8],
                            className: "dt-body-right",
                            render: function (data, type, row) {
                    ...
                    if (controllerReport == "penjualanperkasir") {
                        columnDefs.push({
                    ...
                    if (controllerReport == "penjualanpertanggal") {
                        columnDefs.push({
                            targets: [2, 3, 4, 5, 6, 7, 8],
                            className: "dt-body-right",
                            render: function (data, type, row) {
                    ...
                    $("#tabel_laporan_"+controllerReport).DataTable({
                        ...
                        footerCallback: function( tfoot, data, start, end, display ) {    
                            ...
                            if(response){
                                if (controllerReport == "penjualanperfaktur"){
                                    $td.eq(1).html(Number(response.jumlahitem).toLocaleString('id-ID'));
                                    $td.eq(2).html(formatuang(response.subtotal,'id-ID','IDR'));
                                    $td.eq(3).html(formatuang(response.pajaktoko,'id-ID','IDR'));
                                    $td.eq(4).html(formatuang(response.pajaknegara,'id-ID','IDR'));
                                    $td.eq(5).html(formatuang(response.jumlah,'id-ID','IDR'));
                                }else if (controllerReport == "penjualanperbarang"){
                                    $td.eq(1).html(Number(response.jumlahitem).toLocaleString('id-ID'));
                                    $td.eq(2).html(formatuang(response.totaljual,'id-ID','IDR'));
                                    $td.eq(3).html(formatuang(response.potongan,'id-ID','IDR'));
                                    $td.eq(4).html(formatuang(response.totalbeli,'id-ID','IDR'));
                                    $td.eq(5).html(formatuang(response.totallaba,'id-ID','IDR'));
                                }else if (controllerReport == "penjualanperpelanggan"){
                                    $td.eq(1).html(formatuang(response.tunai,'id-ID','IDR'));
                                    $td.eq(2).html(formatuang(response.kredit,'id-ID','IDR'));
                                    $td.eq(3).html(formatuang(response.transfer,'id-ID','IDR'));
                                    $td.eq(4).html(formatuang(response.kdebit,'id-ID','IDR'));
                                    $td.eq(5).html(formatuang(response.kkredit,'id-ID','IDR'));
                                    $td.eq(6).html(formatuang(response.emoney,'id-ID','IDR'));
                                    $('tr:eq(1) th:eq(1)', this.api().table().footer()).html(formatuang(response.grandtotal,'id-ID','IDR'));
                                }else if (controllerReport == "penjualanperjenistransaksi"){
                                    ...
                                    $td.eq(5).html(formatuang(response.kkredit,'id-ID','IDR'));
                                    $td.eq(6).html(formatuang(response.emoney,'id-ID','IDR'));
                                    $td.eq(7).html(formatuang(response.grandtotal,'id-ID','IDR'));
                                }else if (controllerReport == "penjualanpertanggal"){
                                    $td.eq(1).html(formatuang(response.jumlahitem,'id-ID','IDR'));
                                    $td.eq(2).html(formatuang(response.tunai,'id-ID','IDR'));
                                    $td.eq(3).html(formatuang(response.kredit,'id-ID','IDR'));
                                    $td.eq(4).html(formatuang(response.transfer,'id-ID','IDR'));
                                    $td.eq(5).html(formatuang(response.kdebit,'id-ID','IDR'));
                                    $td.eq(6).html(formatuang(response.kkredit,'id-ID','IDR'));
                                    $td.eq(7).html(formatuang(response.emoney,'id-ID','IDR'));
                                    $td.eq(8).html(formatuang(response.grandtotal,'id-ID','IDR'));
                                }
                            }
                        }
                    });
