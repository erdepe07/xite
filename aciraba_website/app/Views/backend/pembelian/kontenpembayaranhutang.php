...
function loadnotahutang(){
    ...
    function proseshitungtabelhutang() {
        ...
        let numRows = table.rows().count();
        let nominalinput = Number(masukannominal.getNumber());
        let sisa = nominalinput;

        for (let a = 0; a < numRows; a++) {
            let an = new AutoNumeric('#pembayaran' + a, {
                decimalCharacter: ',',
                digitGroupSeparator: '.',
            });

            let sisakredit = Number(
                $("#sisakredit" + a)
                    .html()
                    .replace('Rp', '')
                    .replaceAll('.', '')
            ...
            if (sisa >= sisakredit) {
                // masih cukup → bayar penuh baris ini
                an.set(sisakredit);
                sisa -= sisakredit;
            } else {
                // tidak cukup → bayar sisa terakhir
                an.set(sisa);
                sisa = 0;
            }
        }
        ...
        // Hitung kembalian
        let totalhutang = Number($("#sisahutangbawah").html()
            .replace('Rp&nbsp;', '')
            .replaceAll('.', '')
            .replace(',', '.')
            .trim()
        ...
        if (totalhutang > nominalinput) {
            $("#kembalian").html(formatuang(0, 'id-ID', 'IDR'));
        } else {
            $("#kembalian").html(
                formatuang(nominalinput - totalhutang, 'id-ID', 'IDR')
            );
        }
    }
...
function konfirmasibayarhutang(){
    ...
    swal.fire({
        ...
        if(result.isConfirmed){
            let d = new Date(); let timenow = d.toLocaleTimeString('en-GB', { hour12: false,timeZone: 'Asia/Jakarta'});
            getCsrfTokenCallback(function() {
                $.ajax({
