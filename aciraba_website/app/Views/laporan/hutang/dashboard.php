                ...
                if (controllerReport == "hutangberedar"){ 
                    ...
                    let table = $("#tabel_laporan_"+controllerReport).DataTable({
                        ...
                        columns: [
                            ...
                            { 
                                ...
                                "className": "text-right",
                                "render": function (data, type, row, meta) {
                                    return formatuang((row.TERBAYAR),'id-ID','IDR');
                                },
                            },
                        ],
                        rowGroup: {
                            ...
                            startRender: function ( rows, group ) {
                                return $('<tr style="color:red">')
                                    ...
                                    .append("<td style='vertical-align : middle;'>Terbayar</td>")
                                    .append('</tr>');
                            },
                            endRender: function ( rows, group ) {
                                let totalhutang = 0,totalsisahutang = 0, totalterbayar = 0;
                                $.each(rows.data(), function (index,element) {
                                    totalhutang += Number(element.TOTALKREDIT);
                                    totalsisahutang += Number(element.SISAKREDIT);
                                    totalterbayar += Number(element.TERBAYAR);
                                    totalhutangfooter += Number(element.TOTALKREDIT);
                                    totalsisahutangfooter += Number(element.SISAKREDIT);
                                    totalterbayarfooter += Number(element.TERBAYAR);
                                });
                                return $('<tr style="color:red">')
                                    .append("<td colspan='4' style='text-align: right;vertical-align : middle'>SUB TOTAL</td>")
                                    .append('<td class="text-right">'+formatuang(totalhutang,'id-ID','IDR')+'</td>')
                                    .append('<td class="text-right">'+formatuang(totalsisahutang,'id-ID','IDR')+'</td>')
                                    .append('<td class="text-right">'+formatuang(totalterbayar,'id-ID','IDR')+'</td>')
                                    .append('</tr>');
                            },
                        },
                   ...
                   let table = $("#tabel_laporan_"+controllerReport).DataTable({
                        ...
                        rowGroup: {
                            ...
                            startRender: function ( rows, group ) {
                                return $('<tr class="text-bold text-right" style="color:red">')
                            ...
                            endRender: function ( rows, group ) {
                                ...
                                $.each(rows.data(), function (index,element) {
                                    totalhutang += Number(element.DEBET);
                                    totalsisahutang += Number(element.KREDIT);
