<html>
<head>
    <title>404 Verifikasi Area</title>
...
<div class="content">
	<h3>YUK VERIFIKASI TERLEBIH DAHULU</h3>
	<p>Kami tau kok, anda membutuhkan aplikasi ini untuk berbisnis. Tapi kami ingin memverifikasi kamu apakah kamu beneran pengguna atau tidak. Silahkan lakukan permintaan OTP terus masukin deh OTP yang kita kirim ke WhatsApp kamu. Mudah bukan</p>
	<hr>
	<div class="form-group">
		<div class="input-group">
		<input id="kodeotp" type="text" class="form-control" placeholder="Masukan OTP, Jika belum dapat silahkan klik REQUEST OTP ➡️➡️➡️➡️➡️➡️➡️➡️➡️">
			<div class="input-group-append">
			<button style="cursor: pointer;" id="requestkodeotp" class="input-group-text btn-warning btn">MINTA OTP</button>
			</div>
		</div>
	</div>
	<button id="aktivasikodeotp" class="btn btn-success btn-lg btn-block btn-pill">Aktivasi KODE OTP</button>
	<a href="<?= base_url().'auth/logout' ;?>" class="btn btn-primary btn-lg btn-block btn-pill">Ke Halaman Portal</a>
	<hr>
	<p style="text-align: right;">Website : erayadigital.co.id<br>WhatsApp : 082-257-808-535 (Seira) / 0822-3364-1442 (Ryan)</p>
</div>
...
<script>
let baseurljavascript = "<?=DYBASESEURL;?>";
var csrfName = '<?= csrf_token() ?>'; 
$("#requestkodeotp").on("click", function () {
    $('#requestkodeotp').prop("disabled",true);
    $('#requestkodeotp').html('<i class="fa fa-spin fa-spinner"></i> PROSES MEMINTA OTP');
    getCsrfTokenCallback(function() {  
        $.ajax({
            url:baseurljavascript+'auth/mintaotp',
            type:'POST',
            dataType:'json',
            data: {
            ...
            complete:function(){
                $('#requestkodeotp').prop("disabled",false);
    			$('#requestkodeotp').html('MINTA OTP');
            },
            success:function(response){
               	if (response.respondata.status == true) return toastr["success"]("Hore.. kode OTP sudah terkirim ke WhatsApp anda. Silahkan cek dan JAGA RAHASIA, jangan berikan kepada siapaun OTP-nya OKE... BOSKU");
				if (response.respondata.status == false) return toastr["error"]("Terjadi kesalahan dalam meminta kode OTP untuk aktivasi. Silahkan hubungi ADMINISTRATOR / TIM IT");
            },
...
$("#aktivasikodeotp").on("click", function () {
    $('#aktivasikodeotp').prop("disabled",true);
    $('#aktivasikodeotp').html('<i class="fa fa-spin fa-spinner"></i> PROSES AKTIVASI OTP');
    getCsrfTokenCallback(function() {  
        $.ajax({
            url:baseurljavascript+'auth/verifikasikodeotp',
            type:'POST',
            dataType:'json',
            data: {
                [csrfName]: csrfTokenGlobal,
                JENISOTP: "wa",
				TOKENOTP: $("#kodeotp").val(),
            },
            complete:function(){
                $('#aktivasikodeotp').prop("disabled",false);
    			$('#aktivasikodeotp').html('Aktivasi KODE OTP');
            },
            success:function(response){
				if (response.status == false) return toastr["error"](response.message);
               	if (response.status == true) return toastr["success"](response.message);
            },
            error: function(xhr, status, error) {
                toastr["error"](xhr.responseJSON.message);
            }
