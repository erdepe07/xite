...
class Laporan extends BaseController{
	...
	public function file_penjualan()
	{
		$client = \Config\Services::curlrequest();
		$datapost = [
			'PERIODEAWAL'       => $this->request->getGet('periodeawal'),
			'PERIODEAKHIR'      => $this->request->getGet('periodeakhir'),
			'CARABAYAR'         => $this->request->getGet('carabayar'),
			'OUTLET'            => $this->request->getGet('outlet'),
			'IDPELANGGAN'       => $this->request->getGet('pelanggan'),
			'KODEBARANG'        => $this->request->getGet('barang'),
			'KODESUPLIER'       => $this->request->getGet('supplier'),
			'GROUPELANGGAN'     => $this->request->getGet('group'),
			'KODEKATEGORI'      => $this->request->getGet('kategori'),
			'KONDISI'           => $this->request->getGet('kondisi'),
			'DARISUBPERUSAHAAN' => $this->request->getGet('subperusahaan'),
			'KODEUNIKMEMBER'    => $this->session->get('kodeunikmember'),
			'PDF_PENJUALAN'		=> 1,
			'KEPEMILIKAN'		=> $this->request->getGet('kepemilikan'),
			'FORMAT_FILE' 		=> $this->request->getGet('format_file'),
			'STREAM_FILE' 		=> $this->request->getGet('stream_file'),
		];
		try {
			if ($this->request->getGet('format_file') == 'pdf') {
				$response = $client->request('POST', BASEURLAPI . 'laporan/formatlaporanpenjualan', [
					'headers' => [
				...
				if ($status == 200) {
					...
					return $this->response
						...
						->setBody($pdfContent)
						->setHeader('Content-Disposition', 'inline; filename="laporan_penjualan.pdf"');
				} else {
					return $this->response->setJSON([
						'status' => 'error',
						'message' => 'Gagal menerima PDF dari Node.js (status: ' . $status . ')',
					]);
				}
			}else if ($this->request->getGet('format_file') == 'excel') {
				$response = $client->request('POST', BASEURLAPI . 'laporan/formatlaporanpenjualan', [
					'headers' => [
				...
				$status = $response->getStatusCode();
				if ($status == 200) {
					$excelContent = $response->getBody();
					return $this->response
						->setContentType('application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
						->setBody($excelContent)
						->setHeader('Content-Disposition', 'attachment; filename="laporan_penjualan.xlsx"');
				} else {
					return $this->response->setJSON([
						'status' => 'error',
						'message' => 'Gagal menerima file Excel dari Node.js (status: ' . $status . ')',
					]);
				}
			}
		} catch (\Exception $e) {
	...
	public function file_pembelian()
	{
		$client = \Config\Services::curlrequest();
		$datapost = [
			'PERIODEAWAL'   	=>date('Y-m-d', strtotime($this->request->getGet('periodeawal'))),
			'PERIODEAKHIR'  	=>date('Y-m-d', strtotime($this->request->getGet('periodeakhir'))),
			'CARABAYAR'			=> $this->request->getGet('carabayar'),
			'OUTLET'   			=> $this->request->getGet('outlet'),
			'KODEBARANG'		=> $this->request->getGet('barang'),
			'KODESUPLIER'  		=> $this->request->getGet('suplier'),
			'TEXT_SUPLIER' 		=> $this->request->getGet('text_suplier'),
			'KODEKATEGORI' 		=> $this->request->getGet('kategori'),
			'TEXT_KATEGORI'		=> $this->request->getGet('text_kategori'),
			'KONDISI'			=> $this->request->getGet('kondisi'),
			'KODEUNIKMEMBER' 	=> $this->session->get("kodeunikmember"),
			'FORMAT_FILE'		=> $this->request->getGet('format_file'),
			'STREAM_FILE'		=> $this->request->getGet('stream_file'),
		...
		try {
			if ($this->request->getGet('format_file') == 'pdf') {
				$response = $client->request('POST', BASEURLAPI . 'laporan/formatlaporanpembelian', [
					'headers' => [
						'Accept'        => 'application/pdf',
						'Authorization' => $_ENV['TOKENSCHEME'] . base64_encode($_ENV['TOKENAPI'] . ":::" . $this->session->get('TOKENUSER')),
					],
				...
				if ($status == 200) {
					...
					return $this->response
						->setContentType('application/pdf')
						->setBody($pdfContent)
						->setHeader('Content-Disposition', 'inline; filename="laporan_pembelian.pdf"');
				} else {
					return $this->response->setJSON([
						'status' => 'error',
						'message' => 'Gagal menerima PDF dari Node.js (status: ' . $status . ')',
				...
				if ($status == 200) {
					...
					return $this->response
						->setContentType('application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
						->setBody($excelContent)
						->setHeader('Content-Disposition', 'attachment; filename="laporan_pembelian.xlsx"');
				} else {
					return $this->response->setJSON([
						'status' => 'error',
						'message' => 'Gagal menerima file Excel dari Node.js (status: ' . $status . ')',
	...
	public function formatlaporanpenjualannodatatables(){
		...
		$datapost = [
			...
			'AWAL' => service('request')->getPost('start'),
			'LIMIT' => service('request')->getPost('length'),
		];
	...
	public function formatlaporanpenjualan(){
		...
		$datapost = [
			...
			'DARISUBPERUSAHAAN' => service('request')->getPost('DARISUBPERUSAHAAN'),
			'KODEUNIKMEMBER' => $this->session->get("kodeunikmember"),
			'STREAMPDF' => service('request')->getPost('STREAMPDF'),
		];
		$json_data = $client->request("POST", BASEURLAPI."laporan/formatlaporanpenjualan", [
			"headers" => [
				"Accept" => "application/json",
		...
		if ($datajson->hasiljson[0]->success == "false"){
			...
			$jumlahitem = 0; $subtotal = 0; $pajaktoko = 0; $pajaknegara = 0; $potonganglobal = 0; $jumlah = 0; $totaljual = 0; $totalbeli = 0; $totallaba = 0; $tunai = 0; $uangmuka = 0; $kredit = 0; $transfer = 0; $kdebit = 0; $kkredit = 0; $emoney = 0; $grandtotaljenistrasnaksi = 0;
			if (service('request')->getPost('KONDISI') == "penjualanperfaktur"){
				foreach ($datajson->hasiljson[0]->data as $rowData) {
					$data[] = [
						$rowData->PK_NOTAPENJUALAN,
						$rowData->TGLKELUAR . " " . $rowData->WAKTU,
						$rowData->NAMA,
						$rowData->JUMLAHITEM,
						$rowData->TOTALBELANJA,
						$rowData->PAJAKTOKO,
						$rowData->PAJAKNEGARA,
						$rowData->SUBTOTAL
					];
					$jumlahitem += $rowData->JUMLAHITEM;
					$subtotal += $rowData->TOTALBELANJA;
					$pajaktoko += $rowData->PAJAKTOKO;
					$pajaknegara += $rowData->PAJAKNEGARA;
					$jumlah += $rowData->SUBTOTAL;
				}
				...
				$totalbeli = 0;
				$totallaba = 0;
				$potongan = array_sum(array_column($datajson->hasiljson[0]->data, 'TOTALPOTONGAN'));

				foreach ($datajson->hasiljson[0]->data as $rowData) {
					$hj = $rowData->TOTALJUAL;
					$hb = $rowData->TOTALBELI;

					$data[] = [
						$rowData->FK_BARANG,
						$rowData->NAMABARANG,
						$rowData->NAMAKATEGORI,
						$rowData->JUMLAHITEM,
						$hj,
						$rowData->TOTALPOTONGAN,
						$hb,
						$rowData->LABA
					];
					$jumlahitem += $rowData->JUMLAHITEM;
				...
				for ($no = 0; $no < $datajson->hasiljson[0]->totaldata; $no++) {
					...
					$row[] = $datajson->hasiljson[0]->data[$no]->NOMINALTUNAI;
					$row[] = $datajson->hasiljson[0]->data[$no]->NOMINALKREDIT;
					$row[] = $datajson->hasiljson[0]->data[$no]->NOMINALTRANSFER;
					$row[] = $datajson->hasiljson[0]->data[$no]->NOMINALKDEBIT;
					$row[] = $datajson->hasiljson[0]->data[$no]->NOMINALKARTUKREDIT;
					$row[] = $datajson->hasiljson[0]->data[$no]->NOMINALEMONEY;
					$data[] = $row;
					$tunai = $tunai + $datajson->hasiljson[0]->data[$no]->NOMINALTUNAI;
					$kredit = $kredit + $datajson->hasiljson[0]->data[$no]->NOMINALKREDIT;
					$transfer = $transfer + $datajson->hasiljson[0]->data[$no]->NOMINALTRANSFER;
					$kdebit = $kdebit + $datajson->hasiljson[0]->data[$no]->NOMINALKDEBIT;
					$kkredit = $kkredit + $datajson->hasiljson[0]->data[$no]->NOMINALKARTUKREDIT;
					$emoney = $emoney + $datajson->hasiljson[0]->data[$no]->NOMINALEMONEY;
				}
				$outputDT = [
					"draw" => 1,
					"recordsTotal" => $datajson->hasiljson[0]->totaldata,
					...
					"kkredit" => $kkredit,
					"emoney" => $emoney,
					"grandtotal" => ($tunai+$transfer+$kredit+$kdebit+$kkredit+$emoney),
				];
				...
				for ($no = 0; $no < $datajson->hasiljson[0]->totaldata; $no++) {
					...
					$row[] = $datajson->hasiljson[0]->data[$no]->NOMINALKARTUKREDIT;
					$row[] = $datajson->hasiljson[0]->data[$no]->NOMINALEMONEY;
					$row[] = ($datajson->hasiljson[0]->data[$no]->NOMINALTUNAI + $datajson->hasiljson[0]->data[$no]->NOMINALKREDIT + $datajson->hasiljson[0]->data[$no]->NOMINALTRANSFER + $datajson->hasiljson[0]->data[$no]->NOMINALKDEBIT + $datajson->hasiljson[0]->data[$no]->NOMINALKARTUKREDIT + $datajson->hasiljson[0]->data[$no]->NOMINALEMONEY);
					$data[] = $row;
					$tunai = $tunai + $datajson->hasiljson[0]->data[$no]->NOMINALTUNAI;
					$kredit = $kredit + $datajson->hasiljson[0]->data[$no]->NOMINALKREDIT;
					$transfer = $transfer + $datajson->hasiljson[0]->data[$no]->NOMINALTRANSFER;
				...
				$data = [];
				$grandtotaljenistrasnaksi = 0;
				$rowData = $datajson->hasiljson[0]->data[0];

				$mapping = [
					'TUNAI' => 'TRANSAKSI UANG TUNAI',
					'KREDIT' => 'TOTAL TRANSAKSI KREDIT MEMBER',
					'TRANSFER' => 'TOTAL TRANSAKSI TRANSFER',
					'KARTU_DEBIT' => 'TOTAL TRANSAKSI KARTU DEBIT',
					'KARTU_KREDIT' => 'TOTAL TRANSAKSI KARTU KREDIT',
					'EMONEY' => 'TOTAL TRANSAKSI EMONEY'
				];

				foreach ($mapping as $kolom => $label) {
					$row = [];
					$row[] = "<div style='text-align: right;'><strong>$label</strong></div>";
					$row[] = "<strong>" . formatuang('IDR', $rowData->$kolom, 'Rp ') . "</strong>";

					$grandtotaljenistrasnaksi += $rowData->$kolom;
					$data[] = $row;
				}

				$outputDT = [
					"draw" => 1,
					"recordsTotal" => count($mapping),
				...
				for ($no = 0; $no < $datajson->hasiljson[0]->totaldata; $no++) {
					...
					$row[] = $datajson->hasiljson[0]->data[$no]->STOKBARANGKELUAR;
					$row[] = $datajson->hasiljson[0]->data[$no]->TUNAI;
					$row[] = $datajson->hasiljson[0]->data[$no]->KREDIT;
					$row[] = $datajson->hasiljson[0]->data[$no]->TRANSFER;
					$row[] = $datajson->hasiljson[0]->data[$no]->KARTUDEBIT;
					$row[] = $datajson->hasiljson[0]->data[$no]->KARTUKREDIT;
					$row[] = $datajson->hasiljson[0]->data[$no]->EMONEY;
					$row[] = $datajson->hasiljson[0]->data[$no]->SUBTOTALBARIS;
					$data[] = $row;
					$jumlahitem = $jumlahitem + $datajson->hasiljson[0]->data[$no]->STOKBARANGKELUAR;
					$tunai = $tunai + $datajson->hasiljson[0]->data[$no]->TUNAI;
					$kredit = $kredit + $datajson->hasiljson[0]->data[$no]->KREDIT;
					$transfer = $transfer + $datajson->hasiljson[0]->data[$no]->TRANSFER;
					$kdebit = $kdebit + $datajson->hasiljson[0]->data[$no]->KARTUDEBIT;
					$kkredit = $kkredit + $datajson->hasiljson[0]->data[$no]->KARTUKREDIT;
					$emoney = $emoney + $datajson->hasiljson[0]->data[$no]->EMONEY;
				}
				$outputDT = [
					"draw" => 1,
					"recordsTotal" => $datajson->hasiljson[0]->totaldata,
	...
	public function formatlaporanpembelian_pdf(){
		$client = \Config\Services::curlrequest();
		$datapost = [
			'PERIODEAWAL' => service('request')->getPost('PERIODEAWAL'),
            'PERIODEAKHIR' => service('request')->getPost('PERIODEAKHIR'),
		...
		$datajson = json_decode($json_data->getBody());
		if ($datajson->hasiljson[0]->success){
			if (service('request')->getPost('KONDISI') == "pembelianperfaktur"){
