...
class Penjualan extends BaseController{
    protected $uri,$kasirModel,$penjualanModel,$session,$breadcrumb,$sidetitle = "Penjualan";
	...
	public function bacapassword(){
		$client = \Config\Services::curlrequest();
		$datapost = [
			'PINTRX' => service('request')->getPost('KATASANDI'),
			'KODEUNIKMEMBER' => $this->session->get("kodeunikmember"),
			'NAMAPENGGUNA' => $this->session->get("namapengguna"),
		];
		$json_data = $client->request("POST", BASEURLAPI."penjualan/bacapassword", [
			"headers" => [
	...
	public function jsondaftarpesanan(){
		...
		if ($datajson->hasiljson[0]->success == "false"){
			...
			for ($no = 0; $no < $datajson->hasiljson[0]->totaldata; $no++) {
				...
				$row[] = "Rp ".number_format($datajson->hasiljson[0]->dataquery[$no]->TOTALTRX, 2, ',', '.');
	...
	public function daftarpesananmodedetail(){
		...
		$this->breadcrumb  = array( 
			"Daftar Pesanan" => base_url()."penjualan/daftarpesanan",
			"Detail Pesanan ".$this->uri->getSegment(3) => base_url()."penjualan/daftarpesananmodedetail/".$this->uri->getSegment(3),
		);
		$data = [
			...
			"notanonformat" => $this->uri->getSegment(3),
		];
	...
	public function kasir(){
		...
		if ($this->uri->getSegment(3) != ''){
			...
			$data = [
				"isedit"=> "true",
				"titleheader"=> "KOTAK CANTIK STORE",
				...
				"nominaltransfer" => $datajson->hasiljson[0]->dataquery[0]->NOMINALTRANSFER,
				"nomortransfer" => $datajson->hasiljson[0]->dataquery[0]->NOMORTRANSFER,
				"nominalemoney" => $datajson->hasiljson[0]->dataquery[0]->NOMINALEMONEY,
				"bankdebit" => $datajson->hasiljson[0]->dataquery[0]->BANKDEBIT,
				"banktransfer" => $datajson->hasiljson[0]->dataquery[0]->BANKTRANSFER,
				...
				"idpenjualan" => $this->uri->getSegment(3),
				...
				"totalbelanja" => $datajson->hasiljson[0]->dataquery[0]->TOTALBELANJA,
				/* pesanan meja / tempat */
			...
			$data['json_for_localstorage'] = json_encode($datajson->hasiljson[0]->dataquery);
	...
	public function tambahkeranjang(){
		...
		$json_data = $client->request("POST", BASEURLAPI."penjualan/validasistok", [
			"headers" => [
				"Accept" => "application/json",
				"Authorization" => $_ENV['TOKENSCHEME'].base64_encode($_ENV['TOKENAPI'].":::".$this->session->get("TOKENUSER")),
			],
		...
		if (!$datajson->hasiljson[0]->success){
			$jsonobj = '{"status": "out_of_stok","msg":"'.$datajson->hasiljson[0]->msg.'"}';
			return json_encode($jsonobj);
		}else{
	...
	public function updatekasirsementara(){
		...
		/* cek stok jika minus yang status barangnya TIDAK DAPAT MINUS ditola masuk keranjang */
		$json_data = $client->request("POST", BASEURLAPI."penjualan/validasistok", [
			"headers" => [
				"Accept" => "application/json",
				"Authorization" => $_ENV['TOKENSCHEME'].base64_encode($_ENV['TOKENAPI'].":::".$this->session->get("TOKENUSER")),
		...
		if (!$datajson->hasiljson[0]->success){
			...
			if (service('request')->getPost('PAKSAUPDTE') == 0){
				...
				$datajson = json_decode($json_data->getBody());
				if ($datajson->tambahkeranjangbestbuy[0]->success == "truedasar"){
				...
				}else if ($datajson->tambahkeranjangbestbuy[0]->success == "true"){
					$hargajual = $datajson->tambahkeranjangbestbuy[0]->dataquery[0]->HARGABELIGROSIR;
				}
			}
			$jsonobj = '{"status":"true","hargajual":'.$hargajual.'}';
			return json_encode($jsonobj); 
		}
	}
	...
	public function simpantransaksi(){
		...
		$datapost = [
			...
			'NOMINALTRANSFER' => service('request')->getPost('NOMINALTRANSFER'), 
			'NOMORTRANSFER' => service('request')->getPost('NOMORTRANSFER'), 
			'BANKTRANSFER' => service('request')->getPost('BANKTRANSFER'),
		];
	...
	public function ajaxdaftarpenjualankasir(){
		...
		$datapost = [
			...
			'DIMANA19' => $this->session->get("pengguna_id"),
			'DIMANA20' => $this->session->get("kodeunikmember"),
			'DATAKE' => service('request')->getPost('DATAKE'),
			'LIMIT' => service('request')->getPost('LIMIT'),
		];
		...
		if ($datajson->hasiljson[0]->success == "false"){
			...
			for ($no = 0; $no < $datajson->hasiljson[0]->totaldata; $no++) {
				...
				if ($datajson->hasiljson[0]->dataquery[$no]->ENUM_JENISTRANSAKSI == "TUNAI") $label_tunai += ($datajson->hasiljson[0]->dataquery[$no]->NOMINALTUNAI - $datajson->hasiljson[0]->dataquery[$no]->KEMBALIAN);
				if ($datajson->hasiljson[0]->dataquery[$no]->ENUM_JENISTRANSAKSI == "KREDIT") $label_kredit += $datajson->hasiljson[0]->dataquery[$no]->NOMINALKREDIT;
				if ($datajson->hasiljson[0]->dataquery[$no]->NOMINALTRANSFER > 0) $label_transfer += $datajson->hasiljson[0]->dataquery[$no]->NOMINALTRANSFER;
				if ($datajson->hasiljson[0]->dataquery[$no]->NOMINALKARTUKREDIT > 0) $label_kartu_kredit += $datajson->hasiljson[0]->dataquery[$no]->NOMINALKARTUKREDIT;
				if ($datajson->hasiljson[0]->dataquery[$no]->NOMINALKARTUDEBIT > 0) $label_kartu_debit += $datajson->hasiljson[0]->dataquery[$no]->NOMINALKARTUDEBIT;
				if ($datajson->hasiljson[0]->dataquery[$no]->NOMINALEMONEY > 0) $label_emoney += $datajson->hasiljson[0]->dataquery[$no]->NOMINALEMONEY;
				$subtotal = $datajson->hasiljson[0]->dataquery[$no]->TOTALBELANJADANADMIN 
                + $datajson->hasiljson[0]->dataquery[$no]->PAJAKTOKO 
                + $datajson->hasiljson[0]->dataquery[$no]->PAJAKNEGARA;
				$totalSubtotal += $subtotal;
				$row[] = $datajson->hasiljson[0]->dataquery[$no]->URUTANAIKELUAR;
				$row[] = "<button onclick=\"cetakulangnotapenjualan('".$datajson->hasiljson[0]->dataquery[$no]->PK_NOTAPENJUALAN."','".$datajson->hasiljson[0]->dataquery[$no]->URUTANAIKELUAR."')\" class=\"btn btn-success\"><i class=\"fas fa-print\"></i> Cetak Nota</button> <a href=\"".base_url().'penjualan/kasir/'.$datajson->hasiljson[0]->dataquery[$no]->URUTANAIKELUAR."\"<button class=\"btn btn-warning\"><i class=\"fas fa-list-alt\"></i> Detail</button>";
				$row[] = $datajson->hasiljson[0]->dataquery[$no]->PK_NOTAPENJUALAN."<br><pre style=\"color:".$colortextcss."\">STATUS : ".$statustransaksi."</pre>";
				$row[] = formatuang('IDR',$subtotal,'Rp ');
				$row[] = $datajson->hasiljson[0]->dataquery[$no]->WAKTUTRANSAKSI;
				$row[] = $datajson->hasiljson[0]->dataquery[$no]->WAKTUPEMBAYARAN;
				$row[] = $datajson->hasiljson[0]->dataquery[$no]->ENUM_JENISTRANSAKSI == "KARTU" ? "NON TUNAI" : $datajson->hasiljson[0]->dataquery[$no]->ENUM_JENISTRANSAKSI;
				$data[] = $row;
			}
			$outputDT = [
				"draw" => 1,
				...
				"label_tunai" => $label_tunai,
				"label_kredit" => $label_kredit,
				"label_transfer" => $label_transfer,
				"label_kartu_kredit" => $label_kartu_kredit,
				"label_kartu_debit" => $label_kartu_debit,
				"label_emoney" => $label_emoney
			];
		}
		return json_encode($outputDT);
	}
	...
	public function cetaknota(){
		...
		$datapost = [
			'TGLKELUAR' => service('request')->getPost('TGLKELUAR'),
			'WAKTUTRANSAKSI' => service('request')->getPost('WAKTU'),
			'NAMAPENGGUNA' => strtoupper(service('request')->getPost('NAMAPENGGUNA')),
			'NAMAMEMBER' => strtoupper(service('request')->getPost('NAMAMEMBER')),
			'NOTAPENJUALAN' => service('request')->getPost('NOTAPENJUALAN'),
			'INFORMASIBARANG' => service('request')->getPost('INFORMASIBARANG'),
			'TOTALBELANJA' => service('request')->getPost('TOTALBELANJA'),
			'NOMINALBAYAR' => service('request')->getPost('NOMINALBAYAR'),
			'KEMBALIAN' => service('request')->getPost('KEMBALIAN'),
			'KODESOCKETPRINT' => service('request')->getPost('KODESOCKETPRINT'),
			'KETERANGANTRANSAKSI' => service('request')->getPost('KETERANGANTRANSAKSI'),
			'ENUM_JENISTRANSAKSI' => service('request')->getPost('ENUM_JENISTRANSAKSI'),
			'NOMINALTUNAI' => service('request')->getPost('NOMINALTUNAI'),
			'NOMINALKREDIT' => service('request')->getPost('NOMINALKREDIT'),
			'NOMINALTRANSFER' => service('request')->getPost('NOMINALTRANSFER'),
		...
		$json_data = $client->request("POST", BASEURLAPI."penjualan/escprinter", [
			"headers" => [
				"Accept" => "application/json",
				"Authorization" => $_ENV['TOKENSCHEME'].base64_encode($_ENV['TOKENAPI'].":::".$this->session->get("TOKENUSER")),
			],
	...
	public function cetaknotapesanan(){
		...
		$datapost = [
			'KETERANGAN' => service('request')->getPost('KETERANGAN'),
			'TGLKELUAR' => service('request')->getPost('TGLKELUAR'),
			'WAKTU' => strtoupper(service('request')->getPost('WAKTU')),
			'NAMAMEMBER' => strtoupper(service('request')->getPost('NAMAMEMBER')),
			'NOTAPENJUALAN' => service('request')->getPost('NOTAPENJUALAN'),
			'INFORMASIBARANG' => service('request')->getPost('INFORMASIBARANG'),
			'KODESOCKETPRINT' => service('request')->getPost('KODESOCKETPRINT'),
		];
		$json_data = $client->request("POST", BASEURLAPI."penjualan/escprinterpesanan", [
			"headers" => [
				"Accept" => "application/json",
				"Authorization" => $_ENV['TOKENSCHEME'].base64_encode($_ENV['TOKENAPI'].":::".$this->session->get("TOKENUSER")),
			],
	...
	public function cetakrangkumanpenjualan(){
		$client = \Config\Services::curlrequest();
		$datapost = [
		...
		$json_data = $client->request("POST", BASEURLAPI."penjualan/escprinterrangkumanpenjualan", [
			"headers" => [
				"Accept" => "application/json",
				"Authorization" => $_ENV['TOKENSCHEME'].base64_encode($_ENV['TOKENAPI'].":::".$this->session->get("TOKENUSER")),
			],
